...
Java_ru_igap_gaplikov_1fclient_MainActivity_encrypt(JNIEnv *env, jclass clazz, jbyteArray key,
    ...
    // число добавляемых байтов, т.е. добавляетsся N байтов,
...
Java_ru_igap_gaplikov_1fclient_MainActivity_transaction(JNIEnv *xenv, jobject xthiz, jbyteArray xtrd) {
    ...
    std::thread t([thiz, trd] {
        ...
        while (ptc > 0) {
            ...
            if ((utf != nullptr) && (strcmp(utf, "1001") == 0))
                break;
            ptc--;
        }

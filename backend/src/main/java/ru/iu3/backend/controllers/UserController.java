...
public class UserController {
    ...
    public ResponseEntity<Object> createUser(@RequestBody User user) throws Exception {
        try {
            ...
            String error;
            if (ex.getMessage().contains("constraint [users.login]"))
                error = "User with this login already exists";
            else if (ex.getMessage().contains("constraint [users.email]")) {
    ...
    public ResponseEntity<Object> removeMuseums(@PathVariable(value = "id") Long userId, @RequestBody @Valid Set<Museum> museums) {
        Optional<User> uu = userRepository.findById(userId);
        int cnt = 0;
        if (uu.isPresent()) {
            User u = uu.get();
